...
try:
    ...
    pass
win.title("APunto")
win.minsize(width=1366, height=768)
...
lbl_4.grid(columnspan=2, pady=2, sticky="EW")
# -----------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------
btn_cancelar = tkinter.Button(win, text="CANCELAR", command=cancelar, width=12, font='arial 15 bold',
                              fg=color['l_gris'], height=2)
btn_cancelar.grid(row=8, column=0, pady=15)
btn_cancelar.configure(bg=color['rojo'])
# -----------------------------------------------------------------------------------------
btn_enter = tkinter.Button(win, text="CONTINUAR", command=seguir, width=12, font='arial 15 bold',
                           fg=color['d_gris'], height=2)
btn_enter.grid(row=8, column=1, pady=15)
btn_enter.configure(bg=color['verde'])
# -----------------------------------------------------------------------------------------
try:
    logo2 = tkinter.PhotoImage(file=CONF_APUNTO.logo2_path)
    lbl_6 = tkinter.Label(win, image=logo2, width=1316, height=65, anchor="e")
    lbl_6.grid(pady=10, columnspan=2)
except:
    lbl_6 = tkinter.Label(win, text=f'  {cac_nombre}', width=132, anchor="e", bg='dark blue', font='arial 16',
                          fg=color['celeste'])
    lbl_6.grid(columnspan=2, pady=10, sticky="EW")
