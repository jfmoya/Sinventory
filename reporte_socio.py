...
class RSoc:
    def grabar(self):
        idnum, f1, f2 = self.txt_in11.get(),self.txt_in1.get(),self.txt_in2.get()
        # print (idnum, f1, f2)
        rep = scale_sql_p3.t_socio(idnum, f1, f2)
        if type(rep[0]) == list:
            rep[0].insert(0,(rep[1],))
            path = 'R_'+idnum+'_'+f1+'_'+f2+'.csv'
    ...
    def __init__(self, master):
        ...
        lbl_2 = tkinter.Label(win, text="REPORTE DE ENTREAGS POR SOCIO, POR RANGO DE FECHAS",
                                bg = color[0], font = 'arial 16')
        lbl_2.grid(columnspan = 3, pady=10)

        lbl_21 = tkinter.Label(win, text="# DE SOCIO", bg = color[2],font = 'arial 10')
        lbl_21.grid(column = 0 ,row = 3, pady=5)
        ...
        self.txt_in11 = tkinter.Entry(win, width = 10, font = 'arial 12')
        self.txt_in11.grid(row = 4,column = 0, pady=10)
        self.txt_in11.focus_set()
