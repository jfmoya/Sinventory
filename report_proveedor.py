...
class RepProv:
            ...
            if len(self.lista) != 0:
                ...
                with open(path, 'w', newline='') as r_file:
                    ...
                    for line in self.lista:
                        linea = [str(line[i]).strip() for i in range(6)]
    ...
    def consultar(self):
        ...
        for line in self.lista:
            self.mensaje.insert('end', f'{line[0]};{line[1]};{line[2]};{line[3]};{line[4]};{line[5]}')
    ...
    def __init__(self, master):
        ...
        btn_consultar = tkinter.Button(win, text="GENERAR", command=self.consultar, width=12, font='arial 11',
                                       height=1)
        btn_consultar.grid(row=7, column=0, pady=5)
        btn_consultar.configure(bg=color[0])

        btn_actualizar = tkinter.Button(win, text="EXPORTAR", command=self.exportar, width=12, font='arial 11',
                                        height=1)
        btn_actualizar.grid(row=7, column=1, pady=5)
