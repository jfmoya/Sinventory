...
class EditProv:
    def actualizar(self):
        ...
        if ruc != '':
            if messagebox.askyesno('Apunto', 'Está a punto de actualizar información.\n¿Desea continuar?',
                                   default='no'):
                ms = scale_sql_p3.p_update(ruc, nombre, fecnac, telf, correo)
                self.mensaje.insert(0, f'{ms[0]} -- {ms[1][0]} {ms[1][1]} {ms[1][2]}')

    def consultar(self):
        codigo = self.txt_in6.get()
        # print(codigo)
    ...
    def __init__(self, master):
        ...
        lbl_6 = tkinter.Label(win, text="CODIGO : ", bg=color[7], font='arial 15')
        lbl_6.grid(row=1, column=0, pady=5, padx=10, sticky="EW")
        self.txt_in6 = tkinter.Entry(win, width=6, font='arial 15')
        self.txt_in6.grid(row=1, column=1, columnspan=2, pady=5, sticky="w")
        self.txt_in6.focus_set()
        ...
        btn_consultar = tkinter.Button(win, text="CONSULTAR", command=self.consultar, width=12, font='arial 11',
                                       height=1)
        btn_consultar.grid(row=7, column=1, pady=15)
        ...
        btn_actualizar = tkinter.Button(win, text="ACTUALIZAR", command=self.actualizar, width=12, font='arial 11',
                                        height=1)
        btn_actualizar.grid(row=7, column=2, pady=15)
        ...
        self.mensaje.grid(row=8, columnspan=3, padx=10, pady=10)
