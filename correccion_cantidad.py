...
class CCant:
    def grabar(self):
        t_num, t_idnum, t_weight, new_weight = self.txt_in1.get(),self.txt_in2.get(),self.txt_in3.get(),self.txt_in4.get()
    ##    print (t_num, t_idnum, t_weight, new_weight)
        try:
            float(t_num), float(t_idnum), float(t_weight), float(new_weight)
            self.mensaje.delete(0, 'end')
            msg0 = scale_sql_p3.t_correct(new_weight, t_num, t_idnum, t_weight)
    ##        print (msg0)
            self.mensaje.insert('end', msg0)
            if msg0 != 'Transaccion no puede ser modificada':
                msg1 = t_idnum +'-'+scale_sql_p3.namer(t_idnum)
                new_ticket = scale_sql_p3.entrega(t_idnum)
    ...
    def __init__(self, master):
        ...
        lbl_2 = tkinter.Label(win, text="INGRESE LA INFORMACION DEL PRODUCTO A CORREGIR:",
        ...
        lbl_6 = tkinter.Label(win, text="CANTIDAD LITROS CORRECTA:", bg = color[7],font = 'arial 12')
